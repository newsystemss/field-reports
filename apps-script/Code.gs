// Field Reports Submission Handler
...
const FORM_TOKEN = 'ns-fieldreports-2026';
...
function doPost(e) {
  try {
    ...
    const required = ['firstName', 'lastName', 'email', 'question', 'methods', 'links', 'format', 'learnings', 'outcomes'];
    ...
    const submissionId = 'FR-' + timestamp.getTime().toString(36).toUpperCase();
...
function sendConfirmationEmail(data) {
  ...
  const subject = 'New Stadium: Field Report Submission Received';
  const body = [
...
function sendTeamNotification(data, submissionId, timestamp, sheetRowUrl) {
  ...
  const subject = 'New Field Report: ' + safeName.trim();
  ...
  parts.push('</p>');
  parts.push('<hr>');
  parts.push('<p><strong>Field Report</strong></p>');
  parts.push('<p><strong>1. Driving Question:</strong><br>' + escapeHtmlMultiline(data.question) + '</p>');
  parts.push('<p><strong>2. Methods / Tools:</strong><br>' + escapeHtmlMultiline(data.methods) + '</p>');
  parts.push('<p><strong>3. Links:</strong><br>' + escapeHtmlMultiline(data.links) + '</p>');
  parts.push('<p><strong>4. Ideal Format/Experience:</strong><br>' + escapeHtmlMultiline(data.format) + '</p>');
  parts.push('<p><strong>5. What People Will Learn:</strong><br>' + escapeHtmlMultiline(data.learnings) + '</p>');
  parts.push('<p><strong>6. Outcomes & Side-Effects:</strong><br>' + escapeHtmlMultiline(data.outcomes) + '</p>');
  if (data.other) parts.push('<p><strong>7. Anything else:</strong><br>' + escapeHtmlMultiline(data.other) + '</p>');
  parts.push('<hr>');
  ...
  const plainBody = [
    ...
    '',
    '--- Field Report ---',
    '1. Driving Question:', data.question, '',
    '2. Methods / Tools:', data.methods, '',
    '3. Links:', data.links, '',
    '4. Ideal Format/Experience:', data.format, '',
    '5. What People Will Learn:', data.learnings, '',
    '6. Outcomes & Side-Effects:', data.outcomes,
    data.other ? '' : null,
    data.other ? '7. Anything else:' : null,
    data.other ? data.other : null
  ].filter(function(line) { return line !== null; }).join('\n');

  GmailApp.sendEmail(TEAM_EMAIL, subject, plainBody, {
    name: 'New Stadium Field Reports',
    htmlBody: htmlBody
  });
}

function doGet() {
  return ContentService.createTextOutput('Field Reports submission endpoint is active.')
